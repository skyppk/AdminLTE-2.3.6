    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio || that.options.columns[j].button) {
                    ...
                    type = that.options.columns[j].button ? 'button' : type;
                    if(that.options.columns[j].button){
                        indexname = sprintf('%s',that.options.columns[j].indexname);
                        onclickfunction  = sprintf('%s',that.options.columns[j].click);
                        if(that.options.columns[j].arg!='null'){
                            arg = sprintf('%s',that.options.columns[j].arg);
                            click =  onclickfunction +'(\'' + item[indexname] + '\''
                                + ',' + arg + ')';
                        }else{
                            click =  onclickfunction +'(\'' + item[indexname] + 
                                 '\')';
                        }
                        if  (that.options.columns[j].click == 'null'){
                            click = 'javascript:void(0)'
                        }
                        sprintf('%s',click);
                        btnname = sprintf('%s',that.options.columns[j].btnname);
                        type = 'submit';
